...
function gsea_io(
    ...
    if gene_set_keywords !== nothing
        ...
        gene_set_genes = Dict(
            gene_set => genes_
            for
            (gene_set, genes_) in gene_set_genes if
            any(
                occursin(gene_set_keyword, gene_set)
    ...
    if n_required_gene_set_element !== nothing
        ...
        gene_set_genes = Dict(
            ...
            for
            (gene_set, genes_) in gene_set_genes if
            n_required_gene_set_element < length(intersect(genes_, gene_x_sample[!, 1]))
        )
    ...
    gene_set_x_sample_tsv_file_path =
