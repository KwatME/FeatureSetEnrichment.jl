function compute_set_enrichment(
    ...
    if length(set_elements) < 10
        ...
        element_index =
            Dict(element => index for (element, index) in zip(elements, 1:length(elements)))
    end

    set_enrichment =
        Dict{String, Tuple{Union{Nothing, Vector{Float64}}, Float64, Float64}}()
...
function compute_set_enrichment(
    ...
    for sample in names(element_x_sample)[2:end]
        ...
        set_x_sample[!, sample] = collect(
            set_enrichment[set][set_enrichment_index]
            for set in set_x_sample[!, Symbol("Set")]
        )
    end
